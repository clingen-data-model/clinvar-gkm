#!/bin/bash
# oracle-scv-statement.sh — full-vs-incremental oracle for gkm_scv_statement.
# Builds gkm_scv_statement FULL and INCREMENTAL for the same compare release from the
...
#   Requires: gkm_scv_condition_sets + diff_* already built for COMPARE_DATE, the shared
#   changed-set (gkm_scv_changed) run, and a same-gate baseline release present with the
#   four statement outputs (built by the CURRENT proc — the baseline gkm_dict_scv /
#   gkm_dict_evidence_line / gkm_dict_proposition schema must match, or the merge errors).
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_scv_statement_proc\`(DATE '${DATE}', FALSE)"
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_scv_statement_proc_incremental\`(DATE '${DATE}', FALSE)"
