...
# Requires gkm_change_log + gkm_delta_build to have run for COMPARE.
...
# gkm_scv_condition_sets is tracked+delta'd but NOT a published bundle section -> excluded.
PAIRS=(
  "gkm_dict_sequence_reference key" "gkm_dict_location key" "gkm_dict_allele key"
  "gkm_dict_copy_number_count key" "gkm_dict_copy_number_change key" "gkm_dict_gene key"
...
for pair in "${PAIRS[@]}"; do
  ...
  bq --project_id="$PROJECT" query --quiet --use_legacy_sql=false "
    ...
    WHERE CAST(b.${K} AS STRING) NOT IN (
            SELECT pk FROM \`${CDS}.gkm_change_log\` WHERE table_name='${T}' AND change_type='D')
  ...
  OUT=$(bq --project_id="$PROJECT" query --quiet --use_legacy_sql=false --format=csv \
    "CALL \`clinvar_ingest.gkm_oracle_compare\`('${RECON}','${CDS}','${T}')" | tail -1)
  echo "  ${OUT}"
