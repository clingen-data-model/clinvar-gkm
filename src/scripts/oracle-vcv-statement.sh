#!/bin/bash
# oracle-vcv-statement.sh — full-vs-incremental oracle for gkm_vcv_statement (the 3 VCV
# statement outputs). Builds gkm_vcv_statement FULL and INCREMENTAL for the same compare
...
#   Requires: the 3 vcv agg tables (gkm_vcv_proc / _incremental), variation_archive +
#   diff_* already built for COMPARE_DATE, the shared impacted set (gkm_rcvvcv_changed)
...
# gkm_vcv_statement writes into {S}; to compare two builds we run full, snapshot the 3
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_vcv_statement_proc\`(DATE '${DATE}', FALSE)"
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_vcv_statement_proc_incremental\`(DATE '${DATE}', FALSE)"
