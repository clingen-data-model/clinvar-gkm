#!/bin/bash
# oracle-catvar.sh — full-vs-incremental oracle for gkm_catvar.
# Builds gkm_catvar FULL and INCREMENTAL for the same compare release from the same
...
#   Requires: gkm_vrs + variation_* + diff_* already built for COMPARE_DATE, and a
...
# gkm_catvar writes into {S}; to compare two builds we run full, snapshot the 7 tables
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_catvar_proc\`(DATE '${DATE}', FALSE)"
...
bq query --project_id="$PROJECT_ID" --use_legacy_sql=false \
  "CALL \`clinvar_ingest.gkm_catvar_proc_incremental\`(DATE '${DATE}', FALSE)"
