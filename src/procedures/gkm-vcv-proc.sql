-------------------------------------------------------------------------------
-- gkm_vcv — build the three VCV aggregation-layer outputs from a release:
--   gkm_vcv_classification_agg     (from temp_vcv_base_data + gkm_scv_condition_sets)
--   gkm_vcv_priority_agg           (reads gkm_vcv_classification_agg)
--   gkm_vcv_aggregate_contribution (reads BOTH prior agg tables)
--
-- Three entry points:
--   gkm_vcv_proc(on_date, debug)              -> full rebuild (unchanged behavior)
--   gkm_vcv_proc_incremental(on_date, debug)  -> incremental (carry-forward + merge)
--   gkm_vcv_build(on_date, debug, incremental) -> internal implementation
--
...
--   vcv_impacted_ids produced by gkm_rcvvcv_changed.
--
...
--   impacted-only into {P}.stg_gkm_vcv_* and each layer reads the PRIOR layer's stg
...
-------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE `clinvar_ingest.gkm_vcv_build`(on_date DATE, debug BOOL, incremental BOOL)
BEGIN
  ...
  DO
    ...
    IF baseline_schema IS NOT NULL THEN
      ...
      EXECUTE IMMEDIATE FORMAT("""
        SELECT (SELECT COUNT(*) FROM `%s.INFORMATION_SCHEMA.TABLES`
                WHERE table_name IN ('gkm_vcv_classification_agg','gkm_vcv_priority_agg',
                  'gkm_vcv_aggregate_contribution')) = 3
    ...
    IF eff_incremental THEN
      ...
      SET class_head = '{CT} `{P}.stg_gkm_vcv_classification_agg`';
      SET priority_head = '{CT} `{P}.stg_gkm_vcv_priority_agg`';
      SET agg_head = '{CT} `{P}.stg_gkm_vcv_aggregate_contribution`';
      SET class_src = '`{P}.stg_gkm_vcv_classification_agg`';
      SET priority_src = '`{P}.stg_gkm_vcv_priority_agg`';
    ELSE
      SET pfilter = '';
      SET class_head = 'CREATE OR REPLACE TABLE `{S}.gkm_vcv_classification_agg`';
      SET priority_head = 'CREATE OR REPLACE TABLE `{S}.gkm_vcv_priority_agg`';
      SET agg_head = 'CREATE OR REPLACE TABLE `{S}.gkm_vcv_aggregate_contribution`';
      SET class_src = '`{S}.gkm_vcv_classification_agg`';
      SET priority_src = '`{S}.gkm_vcv_priority_agg`';
    END IF;
    ...
    IF NOT debug THEN
      CALL `clinvar_ingest.cleanup_temp_tables`(rec.schema_name, [
        'temp_vcv_base_data',
        'stg_gkm_vcv_classification_agg',
        'stg_gkm_vcv_priority_agg',
        'stg_gkm_vcv_aggregate_contribution'
      ]);
    END IF;
    ...
    -- temp_vcv_base_data + {S}.gkm_scv_condition_sets).
    -------------------------------------------------------------------------
    ...
    SET query_classification = REPLACE("""
      ...
      vcv_conditions AS (
          ...
          FROM `{P}.temp_vcv_base_data` b
          JOIN `{S}.gkm_scv_condition_sets` scs ON b.scv_id = scs.scv_id
          CROSS JOIN UNNEST([
            COALESCE(
    ...
    IF eff_incremental THEN
      ...
      SET query_merge = REPLACE("""
        CREATE OR REPLACE TABLE `{S}.gkm_vcv_classification_agg` AS
        SELECT
        ...
        FROM `{BASE}.gkm_vcv_classification_agg` b
        ...
        FROM `{P}.stg_gkm_vcv_classification_agg`
      ...
      -- priority
      SET query_merge = REPLACE("""
        CREATE OR REPLACE TABLE `{S}.gkm_vcv_priority_agg` AS
        SELECT
          b.id, b.prop_id, b.variation_id, b.vcv_accession, b.full_vcv_id,
          b.statement_group, b.prop_type, b.submission_level, b.submission_level_label,
        ...
        FROM `{BASE}.gkm_vcv_priority_agg` b
        LEFT JOIN `{S}.vcv_impacted_ids` imp ON imp.vcv_accession = b.vcv_accession
        WHERE imp.vcv_accession IS NULL
          AND b.vcv_accession IN (SELECT id FROM `{S}.variation_archive`)
        UNION ALL
        ...
        FROM `{P}.stg_gkm_vcv_priority_agg`
      """, '{BASE}', baseline_schema);
      SET query_merge = REPLACE(query_merge, '{P}', IF(debug, rec.schema_name, '_SESSION'));
      SET query_merge = REPLACE(query_merge, '{S}', rec.schema_name);
      EXECUTE IMMEDIATE query_merge;
      ...
      SET query_merge = REPLACE("""
        CREATE OR REPLACE TABLE `{S}.gkm_vcv_aggregate_contribution` AS
        SELECT
          b.id, b.prop_id, b.variation_id, b.vcv_accession, b.full_vcv_id,
        ...
        FROM `{BASE}.gkm_vcv_aggregate_contribution` b
        LEFT JOIN `{S}.vcv_impacted_ids` imp ON imp.vcv_accession = b.vcv_accession
        WHERE imp.vcv_accession IS NULL
          AND b.vcv_accession IN (SELECT id FROM `{S}.variation_archive`)
        UNION ALL
        ...
        FROM `{P}.stg_gkm_vcv_aggregate_contribution`
      """, '{BASE}', baseline_schema);
      SET query_merge = REPLACE(query_merge, '{P}', IF(debug, rec.schema_name, '_SESSION'));
      SET query_merge = REPLACE(query_merge, '{S}', rec.schema_name);
      EXECUTE IMMEDIATE query_merge;
    ...
    IF NOT debug THEN
      ...
      IF eff_incremental THEN
        DROP TABLE IF EXISTS _SESSION.stg_gkm_vcv_classification_agg;
        DROP TABLE IF EXISTS _SESSION.stg_gkm_vcv_priority_agg;
        DROP TABLE IF EXISTS _SESSION.stg_gkm_vcv_aggregate_contribution;
      END IF;
    END IF;
...
CREATE OR REPLACE PROCEDURE `clinvar_ingest.gkm_vcv_proc`(on_date DATE, debug BOOL)
BEGIN
  CALL `clinvar_ingest.gkm_vcv_build`(on_date, debug, FALSE);
END;
...
CREATE OR REPLACE PROCEDURE `clinvar_ingest.gkm_vcv_proc_incremental`(on_date DATE, debug BOOL)
BEGIN
  CALL `clinvar_ingest.gkm_vcv_build`(on_date, debug, TRUE);
END;
