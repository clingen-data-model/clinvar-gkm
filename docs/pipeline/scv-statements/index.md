...
**Output:** `temp_gkm_scv` — one row per SCV with core classification and submitter metadata. <span class="role-badge badge-internal">Internal</span>
...
### Step 5: Build `temp_gkm_scv_proposition`

Assembles primary SCV propositions by joining SCV records with gene context, MOI, penetrance qualifiers and condition sets from `gkm_scv_condition_sets`.
...
### Step 6: Build `temp_gkm_scv_target_proposition`
...
**Output:** `gkm_dict_scv` — one row per SCV with complete VA-Spec Statement record. <span class="role-badge badge-pipeline">Pipeline table</span>
...
|---|---|---|
| `temp_gkm_scv` | Core SCV records with classification and submitter metadata | <span class="role-badge badge-internal">Internal</span> |
...
| `temp_gkm_scv_proposition` | Assembled primary propositions with qualifiers and conditions | <span class="role-badge badge-internal">Internal</span> |
| `temp_gkm_scv_target_proposition` | Somatic target propositions for clinical impact assertions | <span class="role-badge badge-internal">Internal</span> |
| `gkm_dict_scv` | Complete VA-Spec Statement records for all SCVs | <span class="role-badge badge-pipeline">Pipeline table</span> |
...
- **Upstream Procedures**: `gkm_scv_condition_proc` (provides `gkm_scv_condition_sets`), `gkm_catvar_proc`
